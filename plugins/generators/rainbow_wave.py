...
_SHADER_PATH = os.path.join(
    os.path.dirname(__file__), '..', '..', 'src', 'modules', 'gpu', 'shaders', 'gen_rainbow.wgsl'
)
...
class RainbowWaveGenerator(PluginBase):
    ...
    def get_shader(self) -> str | None:
        if RainbowWaveGenerator._shader_src is None:
            with open(_SHADER_PATH, 'r', encoding='utf-8') as f:
                RainbowWaveGenerator._shader_src = f.read()
        return RainbowWaveGenerator._shader_src
    ...
    def get_uniforms(self, **kwargs) -> dict:
        return {
            ...
            'vertical':    int(bool(self.vertical)),
        }
